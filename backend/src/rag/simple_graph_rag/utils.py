...
class CypherQueryCorrector:
    ...
    def fix_multiple_relationship_types(self, query: str) -> str:
        """
        Fix incorrect syntax for multiple relationship types.
        Changes [:TYPE1|:TYPE2] to [:TYPE1|TYPE2]
        ...
        Returns:
            Fixed cypher query
        """
        # Pattern to match [:TYPE1|:TYPE2|:TYPE3...]
        # Captures the entire relationship pattern in brackets
        pattern = r'\[([^\]]+)\]'
        ...
        def fix_relationship(match):
            ...
            # Find the first part that starts with :
            first_type_idx = None
            ...
            if first_type_idx is None:
                # No colons found, return original
                return match.group(0)

            # Remove leading : from all parts except the first one
            fixed_parts = []
            for i, part in enumerate(parts):
                stripped = part.strip()
                if i > first_type_idx and stripped.startswith(':'):
                    # Remove the leading colon
                    fixed_parts.append(stripped[1:] if len(stripped) > 1 else '')
                else:
                    fixed_parts.append(part)

            return '[' + '|'.join(fixed_parts) + ']'

        # Apply the fix to all relationship patterns
        fixed_query = re.sub(pattern, fix_relationship, query)
    ...
    def correct_query(self, query: str) -> str:
        ...
        Args:
            query: cypher query
        """
        # Step 1: Fix multiple relationship types syntax
        query = self.fix_multiple_relationship_types(query)
