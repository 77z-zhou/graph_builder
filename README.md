...
- 两个测试界面：
  - `test.html` - 完整的 API 测试平台
  - `graph-chat.html` - Graph RAG 聊天界面
...
然后访问：
- **API 测试平台**：http://localhost:8000/test.html
- **Graph RAG 聊天**：http://localhost:8000/graph-chat.html
...
1. 打开 `frontend/test.html` 或 `frontend/graph-chat.html`
2. 填写 Neo4j 连接信息
3. 点击"测试连接"按钮

#### 2. 上传文档

1. 选择 LLM 模型（DeepSeek 或 Qwen）
2. 拖拽或点击选择文件（支持 PDF、TXT、DOCX）
3. 点击"上传文件"
...
1. 打开 `frontend/graph-chat.html`
2. 选择聊天模式：
   - **Generate Cypher**：AI 自动生成 Cypher 查询语句
   - **Graph Retrieve**：基于向量相似度检索相关内容
3. 输入问题并开始对话
...
2. 检查浏览器控制台的 CORS 错误
3. 确认前端 HTML 中的 `API_BASE_URL` 配置正确
